namespace Shooter3D
{
    ...
    public class PixelShooter3D : Form
    {
        ...
        {
            // Handle rotation (fixed direction)
            ...
            if (moveDirection != Vector3.Zero)
            {
                ...
                // Transform movement direction by camera rotation
        ...
        {
            var projectedObjects = new List<(Mesh mesh, Dictionary<int, Point> projectedPoints, List<int[]> faces)>();

            foreach (var obj in worldObjects)
            {
                // Map from original vertex index to projected Point
                var projectedDict = new Dictionary<int, Point>();
                for (int i = 0; i < obj.Vertices.Count; i++)
                {
                    var worldPos = obj.Vertices[i] + obj.Position;
                    var projected = ProjectPoint(worldPos);
                    if (projected.HasValue)
                    {
                        projectedDict[i] = projected.Value;
                    }
                }

                if (projectedDict.Count > 0)
                {
                    projectedObjects.Add((obj, projectedDict, obj.Faces));
                }
            }
            ...
            foreach (var obj in projectedObjects)
            {
                ...
                {
                    // Only draw if all face indices are present in the projectedDict
                    if (face.All(idx => obj.projectedPoints.ContainsKey(idx)))
                    {
                        ...
                        if (points.Length >= 3)
                        {
                            ...
                            {
                                // Optional: log or break here for debugging
                                System.Diagnostics.Debug.WriteLine($"Exception drawing face: {ex.Message}");
                            }
                        }
                    }
                    // else: skip this face, as not all points are valid
                }
            }
        }
